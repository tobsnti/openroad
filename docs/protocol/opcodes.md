...
Small S→C pushes the vSRO 1.188 server sends at world join
(PR #179). Typed so the fan-out exists;
...
| `0x3305` | FriendListInfo | S→C | experimental | friend roster; only ever seen empty; EP-32 / #154 (social) |
...
| `0x70CB` | PetMountRequest | C→S | experimental | `{u8 mount_state, u32 cos_unique_id}` — **byte first**. Previously wired as a 4-byte guess, which was wrong in both width and order |
