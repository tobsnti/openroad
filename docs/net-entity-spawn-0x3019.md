...
union-id, union-crest-rev, is_friendly and siege_authority (not "3×u32
crest/union revs").
...
dropper_uid:u32` pair belongs to the **0x3015 frame**, not to
...
A "is the tail there?" heuristic — classify the next four bytes, and
...
- **COS records.** COS refs (characterdata `1/2/3/tid4`) now parse with their
  ...
  A plain NPC parse leaves that tail unconsumed and desyncs every later record
  in the batch.
