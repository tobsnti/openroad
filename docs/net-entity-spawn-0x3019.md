...
34067 -> 36031), and every entity behind such a record was lost with it. The
record's *layout* is still unknown, but its *width* is decidable from the
payload: assume an `n`-byte body, parse the records that must follow, and keep
`n` only if they all parse and land exactly on the payload's last byte — a
group-spawn body holds whole records and nothing else, so each payload is
...
- **Player mount/transport.** `parse_player` now models the conditional: a set
  ...
  on the wire. The old flat 8-byte skip desynced the batch by 4 bytes for any
  mounted player.
