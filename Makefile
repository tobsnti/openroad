...
# Leak gate: runs on the PATCH, not the tree. This fork legitimately holds
# material that may never cross into a public repository (server endpoints,
# vendor filenames, the archive key), so the question is what a diff adds, not
# what the checkout contains. Override the base with `make no-private LEAK_BASE=<ref>`.
