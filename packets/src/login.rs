...
/// `result == 2, error_code == 1` settles it. Left unchanged rather than
/// guessed at.
...
mod tests {
    ...
    /// All 15 codes from the UI pump's switch (the original's login switch) are
    /// named, and anything else degrades to `Unknown` instead of being silently
    /// dropped.
    #[test]
