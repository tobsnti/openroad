...
// COUNTING RULE for this table. The line-anchored grep below and the
// macro-block parser in scripts/check_opcode_ledger.py return the same set.
// One entry below = one wired opcode:
//
...
packets! {
    ...
    // bytes each way: u8 kind, u8 sub, u8[0x400].
    ...
    // 0x7060 pairs with 0xB060 and 0x7062 with 0xB062, so the invite ack is
    // 0xB062.
    ...
    // Player exchange / trade (see agent/exchange.rs). Layouts are unconfirmed
    // on the wire.
    ...
    // the 0xB04C response is unconfirmed. See agent/inventory.rs.
    0x704C => ItemUseRequest,
    0xB04C => ItemUseResponse,

    // Storage (confirmed; see agent/storage.rs). The push
    ...
    // read from the original's builders and handlers; none of the seven
    ...
    // Mail/memo + consignment "avatar market" (see agent/mail.rs). Layouts are
    // unconfirmed on the wire. 0xB509's records are variable-length and their width
