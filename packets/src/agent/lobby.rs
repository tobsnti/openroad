...
pub struct CharacterSelectionActionRequest {
    ...
    /// `05 01`. The shape is identical across actions 3, 4 and 5. Leaving
    /// `Restore` out of this gate sent a bare `[05]` with the name silently
    /// dropped; the byte tests below pin all of it.
    #[sro_packet(
...
pub fn lobby_error_text(code: u16) -> LobbyErrorText {
    // The in-range default; it is *not* what
    // out-of-range codes get, which is the one thing §3.5 used to state wrongly.
