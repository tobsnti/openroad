...
///
/// Removing the fields outright is the right end state; it touches seven other
/// files (`login_form.rs`, `server_select.rs`, `captcha.rs`,
/// `character_create.rs`, `character_select.rs`, `net.rs`, `region_select.rs`).
///
    ...
    fn build(&self, app: &mut App) {
        app.add_systems(
            ...
            (
                ...
                // so each one is gated on its own message store.
...
mod test {
    ...
    /// The click still sounds — and both variants can, which `first()` would
    /// not allow (`uibutton_b.wav` would be unreachable).
    #[test]
