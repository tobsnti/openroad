...
/// camera's working range (`camera.rs`: 40..400); 0 means "never cull", which
/// stays reachable but is not a step anyone lands on by accident.
...
impl ExtraRow {
    ...
    /// setting applies instantly and is then lost on the next launch. Saying so
    /// in the value is the same honesty idiom this pane already uses for
    /// `(not applied)` and `(no effect yet)`; mirroring the three fields into
    /// [`GameOptions`] is the real fix and is a bigger change than this row
    /// deserves.
...
// The window itself is applied by `plugins::config::window`, which resolves one
// `WindowIntent` from `config.yaml` plus the session-only
// `video.window_mode_override` and is the single writer for `window.mode`.
...
mod tests {
    ...
    /// that applies instantly and is gone next launch must say so in the row.
    #[test]
