...
/// **every HUD button becomes audible at once**.
/// The old query filtered on `&ButtonSound`, a component that is set in
/// `scenes/intro_v2/**` and nowhere else, so all 57 HUD files' buttons were
/// silent while every pregame button was not.
///
...
fn play_button_click_sound(
    ...
    match (&effect_sounds, table_has_click) {
        ...
        // reachable — the old `first()` here could only ever play `_a`.
...
mod tests {
    ...
    fn click_app(enabled: bool, fx_enabled: bool, table: bool) -> (App, Entity) {
        ...
        // HUD files, and the old `Query<&ButtonSound, Added<Pressed>>` is the
        // reason every one of them was silent.
    ...
    /// Pressing is not the click. Ours used to fire here (`Added<Pressed>`),
    /// which was a deviation from the original.
    #[test]
