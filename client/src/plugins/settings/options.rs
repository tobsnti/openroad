...
impl GraphicProfile {
    ...
    /// touched this" — the same shape that made `window_mode: bool` stamp
    /// borderless fullscreen over `config.yaml` (see
    /// [`VideoOptions::window_mode_override`]).
    /// Restoring a saved resolution at startup therefore treats *the default pair*
    /// as "unset" and leaves the window to `config.yaml`. That is a deliberate,
    /// stated deviation, not a guess: the alternative is either ignoring the
    /// player's saved size forever or forcing 1920x1080 on every fresh install.
    /// The cost is one case — a player whose chosen size happens to equal the
    /// shipped default keeps `config.yaml`'s window.
...
/// # The idea
///
/// Before this, `GameplayOptions::default()` was an *empty* map and every
/// reader fell back to `unwrap_or(true)` — "on" for all 27 boxes, which is not
/// what the original ships: seven of them (HideAbilityPets, the HP/MP
/// warnings, the four condition bars) and two more (2026 / 2028) ship *off*.
/// That fallback was the last unsourced magic number in this module, so the
/// table replaces it and [`default_toggle`] is the single place a missing id
/// is answered from.
///
/// Four of these have visible HUD consequences: 2021/2022/2023 and 2024 ship
/// **off**. 2024 had the one live consumer until the over-head condition bar
/// (`hud/quickstate.rs`) was removed (ADR 0009, `do-not-wire`); the shipped
/// value stays as the file states it, so a rebuild inherits it. That is the
/// shipped state, not a preference; the one place to flip it is the table
/// below.
///
...
/// resurrect the config-clobber documented on that field.
...
mod tests {
    ...
    /// An older `user_settings.yaml` written when the map was empty must not
    /// come back as "no toggles at all": `serde(default)` on that field would
    /// have produced an empty map, i.e. the old blanket-on behaviour.
    #[test]
    ...
    /// 1001/1002/1003 = 30/50/50 and 1004-1006 = 1/1/1. Unity gain was the old,
    /// invented default; a change back to it should have to edit this test and
    /// say why.
    #[test]
