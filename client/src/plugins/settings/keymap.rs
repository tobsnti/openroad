...
mod tests {
    ...
    /// A focused text field must not fire shortcuts: typing a name like
    /// "Uigur run" would otherwise trigger Inventory, Skill and the match board
    /// itself as the letters go by, because every toggle gates only on the
