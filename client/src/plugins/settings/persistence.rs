...
///
/// Idea: the importer used to be a `pub fn` with zero call sites, held in the
/// build by `#[allow(dead_code)]` — an entry point that reads as done while
/// never having been executed against anything. It needed *a* caller, and an
/// env var is the cheapest honest one: it is the same idiom the rest of this
/// client already uses for operator-facing switches (`USER_SETTINGS_PATH`
/// above, `RESOLUTION`, `NETCHECK`, `SCREENSHOT`), it needs no UI, and unlike
/// a cargo feature it actually *runs*, which is the whole point.
///
...
mod tests {
    ...
    /// The importer had zero call sites for its whole life, so this is also the
    /// first time it is executed against anything at all.
    #[test]
