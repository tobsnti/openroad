...
enum CursorKind {
    ...
    /// mode visible.
    ///
    /// ⚠️ Which slice: the amulet is a choice of OURS. Media ships eight
...
/// silently never changes. `cursors/Cursor14.png` was exactly that for as long
/// as it was referenced. The
/// table is what `every_cursor_image_is_shipped` walks.
///
...
mod tests {
    ...
    /// and that cursor kind is simply invisible. `cursors/Cursor14.png` was
    /// referenced without ever being cropped out of the sheet, which is how a
    /// fresh Windows install greeted every start with an asset error.
    #[test]
