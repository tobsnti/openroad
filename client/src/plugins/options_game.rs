...
/// wide, 6 px to the checkbox, checkbox 16). Without it the label wraps to two
/// lines inside a 24 px row and overprints the row below it.
...
/// above this or the labels overlap.
#[cfg(test)]
...
const GAME_SET_ROWS: [ToggleRow; 7] = [
    ...
    // Inert like its three siblings: openroad shows vitality over the head
    // instead of a quick-state widget, so nothing in the tree reads 2024 and a
    // clickable switch here would be a dead wire.
    ToggleRow {
        id: Some(2024),
...
/// two "refuse" switches of the Community group.
///
...
pub(crate) fn spawn_game_pane(
    ...
    .enumerate()
    {
        ...
        // it clips *and* scrolls. The rows are laid out in flow rather than
        // absolutely on purpose: bevy derives the scrollable content size from
        // the in-flow children, so an absolutely positioned row stack reports a
        // content height of zero and layout clamps every `ScrollPosition` back
        // to 0. Same top-to-bottom 24 px pitch either way.
        pane.spawn((
            Node {
                position_type: PositionType::Absolute,
...
/// parser for `SROptionSet.dat`, which is otherwise only reachable through an
/// environment variable read once at boot — a migration, but not something a
/// player can try and take back.
///
...
/// partly: a row whose checkbox is half cut off is not a reachable control.
/// Test-only — bevy does the clamping at runtime, but this pane's reachability
/// claim is arithmetic over `ROW_H` and the authored viewport, so it is
/// checkable without a layout pass.
#[cfg(test)]
...
mod test {
    ...
    /// `gameplay.toggles`. 2024 is deliberately absent: nothing reads it.
    ...
    /// The two Community ids exist here **because**
    ...
    /// **The reachability proof.** Nine rows in a 101 px viewport means rows
    /// 5..9 — both Community switches among them — are drawn and then clipped
    /// away, so finding an id in the row table proves nothing. This test is
    /// about *geometry after scrolling* instead.
    #[test]
    fn the_community_switches_are_reachable_by_scrolling() {
        ...
        // The upper box has the same shape: six rows in the same 101 px, so
        // `Guild Name` (2014, a live nameplate id) and `Beginner's Mark` are
        // only reachable by scrolling too.
    ...
    /// row, so a second line lands on the row below. `Monster Name`, `Self
    /// Condition`, `COS Condition`, `Party Member Status` and `Monster
    /// Condition` are the labels at risk.
    #[test]
