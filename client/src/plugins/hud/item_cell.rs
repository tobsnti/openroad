...
//! impression that the window is dead.
//!
...
impl HoveredItem {
    ...
    /// Clear only a *catalog* entry.
    ///
    /// The shop is the sole publisher of [`HoveredItemKind::Catalog`], and it
    /// runs every frame — also with no shop open. Calling [`Self::clear`]
    /// there erased whatever the warehouse or the inventory had just
    /// published, one writer against another, and the tooltip flickered. A
    /// publisher only retracts what it published.
