...
//!
//! Absence claim with a positive control, on the user's own
//! `Media/server_dep/silkroad/textdata/` (all 154 files, 2026-08-21): the
...
///
/// `[V]` census over the user's own `itemdata*.txt` (10 shards, 12 061 enabled
/// rows, 2026-08-21): all 43 `(3,3,8,1|2)` rows have price 383 and sell price
...
/// exactly the ones that occur (census 2026-08-21). Region → town mapping from
...
pub struct SpecialtyGood {
    ...
    pub generation: u32,
    /// Column 57; 40 on all 43 rows (`[V]`, census 2026-08-21) — the sack
