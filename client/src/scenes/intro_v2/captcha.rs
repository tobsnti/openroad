...
/// which is 200x64 at 1bpp = 1600 bytes.
...
/// arrives.
///
/// `on_captcha_confirm_response` keeps the modal up on a rejection (the user
/// retypes), but a gateway that answers the rejection with a fresh `0x2322`
/// re-inserts [`CaptchaImageV2`], and `spawn_captcha` hangs on `resource_added`
/// — so the second challenge would spawn a *second* modal on top of the first.
/// Two modals mean two [`CaptchaInput`] entities, and both
