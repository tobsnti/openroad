...
/// Everything the interface data authors *per race*, for the races our
/// reference corpus authors. Each field keeps the citation it had when it was
/// a match arm.
...
mod tests {
    ...
    /// single-race corpora — (a) is the maintainer's own, (b) its mirror image
    /// that some other server ships; (c) is a corpus with both; (d) is the
    /// point of the whole module: a race whose code this program has never
    /// heard of must appear, because its bodies are in the table.
    #[test]
    ...
    /// cite keep the values they had as match arms.
    #[test]
