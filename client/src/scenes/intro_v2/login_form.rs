...
/// one main button the original leaves un-`#ifdef`ed.
...
/// `InteractionDisabled` on [`ServerListButton`] — but an empty slot makes
/// `ui_v2::update_image_button_visuals` fall back to the normal frame with a
/// `warn_once`, i.e. a shipped asset would go unused for no stated reason.
...
// spells `k` out). The rects are taken verbatim and turned into percentages of
// the same `DesignFit` box the loading chrome uses, so neither `k` nor a pixel
// size is written here.
//
// `GDR_STA_LOGO` id 8, `Rect="540,403,508,172"`, `logo.ddj`
// (`pstitle_europe.txt:600/605`): at 800x600 that is **254x86 at (270,201)**,
// half the art's native 508x172. Its bottom, 201+86 = 287, lands 19 px above
// the window's top edge 306.
...
// The original paints it in the top left of the login screen.
const TITLE_RECT: (f32, f32, f32, f32) = (47.0, 110.0, 184.0, 36.0);

// The gap the original draws. `GDR_BTN_OK`/`GDR_BTN_CANCEL` are `Rect="0,0,91,41"`
...
// 288-wide frame would spread them ~35 px apart — a number nothing authors.
...
/// `bevy_ui_widgets` fires `Activate` for a **focused** button on Enter, so an
/// Exit one Tab past Connect quits the client for a player who tabs and presses
/// Enter, with nothing logged.
///
...
///
/// The `TabGroup` is **not** authored on the root; it is inserted while the
/// login form is the screen on show and removed when it stops being that.
/// `hide_screen` (`fade.rs`) only fades and sets `Visibility::Hidden`, and
/// nothing despawns `LoginFormRoot`. Bevy 0.19 collects Tab targets **purely
/// structurally**: `TabNavigation`
...
/// presses Tab would land in the invisible login form, and one Enter on the
/// invisible list button would throw the session back to `ServerSelection`.
///
...
/// was aimed at the screen it replaces. The case it covers is a double-click on
...
pub fn login_form(
    ...
    // so. Vertical *centring* would put it at 230 — 76 px too high.
    // Horizontally the original *is* centred: `(800-288)/2 = 256 ~= 255`, so
    // that half stays an alignment rather than a number.
            ...
            (
                ...
                Children [
                    ...
                    (
                        ...
                        // a clean 28px pitch, 32/60/88. The *label* tops from
                        // `GDR_STATIC1/2` (`:120`, `:101`) are 35/62 and belong
                        // to the other column. In the original the field content
                        // rows start at y 338/366/394 against a window origin of
                        // (256,306), i.e. 32/60/88.
...
/// player's say-so. Quitting silently leaves nothing in the log to say who did
/// it; one line makes an accidental quit self-diagnosing.
pub fn quit_app(
...
/// openroad affordance**, not a reproduction: without it the screen cannot be
/// typed into at all until the player finds the field with the mouse, and the
/// field it picks is the one the original draws first (`GDR_EDIT_ID`,
/// `pstitle_europe.txt:63`).
///
...
///
/// **A deviation, on an unknown.** What the original does with Enter on a
/// filled form is **not established**. We submit on Enter because a login form
/// that cannot be completed from the keyboard is a usability defect, and
/// whether the original agrees is open. It goes through `Activate` rather than
/// duplicating [`super::net::on_connect_activate`], so a disabled Connect
/// (request in flight) stays disabled and the sound/observer path is the one
/// the mouse takes.
...
/// by the response, so giving it a second owner would race that one.
...
mod tests {
    ...
    /// The title caption is authored into the scene. Source scan for the reason
    /// the file's other markup tests give: no test app can spawn this scene.
    #[test]
    ...
    fn connect_and_exit_sit_18px_apart_centred_on_the_screen() {
        ...
        // Red control: `SpaceEvenly` puts three equal gaps in the row, i.e.
        // ~35 px between the buttons, not 18.
    ...
    /// replaces, and Exit is the button where that matters most: a double-click
    /// on the server window's Select, whose area overlaps the form's button row,
    /// puts the second click on this row half a second before it is visible.
    ///
    ...
    /// form.** The form is hidden, never despawned, and `TabNavigation` filters
    /// on neither `Visibility` nor `InteractionDisabled` — so the only thing
    /// that takes its members out of the ring is the `TabGroup` going away with
    /// the screen.
    ///
    ...
    /// `NoTabGroups` afterwards is the rule and not a broken query.
    #[test]
    ...
    /// The state guard both actions of this screen ask before they move the
    /// state machine. Red control included — the same helper answers `true` for
...
",
    ...
    /// What this pins: the "client closes itself" path is the *Exit button*
    /// activated from the keyboard — `bevy_ui_widgets` activates a focused
    /// button on Enter, and Exit one `TabIndex` past Connect is one Tab plus
    /// Enter away. So the authored Exit block must carry no `TabIndex`, and the
    /// ring must stop at Connect.
    ///
    /// Source scan: what has to hold is a property of the authored markup, and
    /// no test app can spawn this scene (it needs `IntroV2Assets` + `FontAssets`
    /// from the PK2).
    #[test]
