...
pub struct CharCreateSelection {
    ...
    pub figure: usize,
    pub weapon: usize,
    /// Height step, `0..=SCALE_STEPS - 1`; the low nibble of the wire scale
    /// byte.
    pub height: u8,
    ...
    /// weapon ref in `this+0x138` and it starts at 0; the two pre-send gates
    /// refuse while it is 0, which is why a fresh screen with the visually
    /// obvious default still answers "Select a Weapon." — see [`gate_selection`].
