...
///
/// Why the art and not only the text fades: `china.ddj`/`europe.ddj` are the
/// description boxes — 440x152 gold-framed dark panels with a tab — so hiding
/// the text child leaves two empty frames standing. The original ramps the
/// whole control's alpha and starts both plates at alpha 0.
#[derive(Component, Clone, Copy)]
...
/// Alpha per second while the click's confirm is running — a different number
/// from the hover ramp: the confirm fades both plates out over `0.5 s`, so the
/// rate is `1 / 0.5`. The hover rate would be a third of a second — faster than
/// the original and indistinguishable from "the pointer left the idol".
...
///
/// Its timer is not the progress source: the gauge counts the creation
/// screen's own asset load, and this timer only starts once that load is full —
/// it is the dwell that lets `OnEnter(CharacterCreate)` build the screen behind
/// a finished loading picture instead of showing one empty frame of stage.
/// Ticked in `CharacterCreate` by [`tick_loading_cut`].
#[derive(Component)]
...
/// Modelling the sequence as one resource with one [`RegionConfirm::advance`]
/// step — rather than as three systems that each guess where they are — is what
/// makes the order testable without a window.
#[derive(Resource)]
...
/// Plate and loading textures are authored per race and therefore live with the
/// race they belong to, in [`super::race_catalog::KNOWN_RACES`] — this screen
/// holds no constant per race, because it does not know how many races there
/// are.
...
/// (`:48`, `:6`), and `GDR_BTN_CANCEL` is `FontIndex=2` (`:706`).
...
///
/// The player-facing plate carries the original's message, not our diagnosis:
/// "your PK2 ships the models but no playable body row" is a note to us, not a
/// message to a player. It is not lost — it goes to a log line at the click,
...
///
/// This is the original's state-8 tick and nothing else: the cursor is turned
/// into a ray through the stage camera and tested against the two idols' own
/// boxes. It deliberately does not go through bevy's picking backends — the
/// plates sit in front of the figures as UI, and the original's own test is a
/// geometric one against exactly two props, with the lizard excluded.
///
...
/// creation selection with the race and start the click's camera move. The
/// original's confirm starts a 1.0 s flight and only its state 9, after the
/// move has landed, shows the loading picture — so everything after the move
/// lives in [`tick_region_confirm`].
///
/// A free function rather than an observer, because the trigger is a
/// pointer-plus-click on the figure rather than a plate button, while this
/// half — what happens once a race is chosen — is independent of that.
#[allow(clippy::too_many_arguments)]
fn enter_creation_for(
    ...
    if super::race_stage::CONFIRM_FADE_TO_BLACK {
        // The original hides its own move under this; see
        // `race_stage::CONFIRM_FADE_TO_BLACK`.
...
pub fn tick_region_confirm(
    ...
    match confirm.advance(time.delta(), settled, total) {
        ...
        ConfirmStep::Done => {
            ...
            // `resource_exists_and_changed::<CharCreateSelection>`. With the
            // original's order the click is a second and a load earlier than
            // the hand-over, so the change would be stale by then and the
            // podium would stay empty. The hand-over is the moment the
            // selection becomes news for the creation screen, so it is stamped
            // here.
            if let Some(mut selection) = selection {
                selection.set_changed();
            }
            // Lift the confirm's fade. The original's `GDR_FADE` is a control
            // it owns and drops with the screen; ours is the shared
            // `FadeScreenV2` overlay, which keeps whatever the tween left on it
            // — otherwise the creation screen stays black. The loading art
            // (z 500) is over the overlay from the moment it spawns, so
            // clearing it here is invisible.
            ...
            // asks (`scenes/intro_v2/mod.rs`). Without it the camera is
...
fn spawn_loading_cut(
    ...
    commands
        ...
        .with_children(|cut| {
            // Art and chrome from the module that owns the rects, so the frame
            // and its caption stay in one design space at any window size. The
            // art lives in the 4:3 cover box — a 1024x768 painting stretched to
            // a 16:9 window is visibly squashed
            // (`loading_screen::DESIGN_ASPECT`).
            //
...
mod tests {
    ...
    /// The order: the loading picture may not appear before the click's camera
    /// move has landed, and its gauge starts at zero on a real count.
    ///
    /// The whole rule in one test: the original's state 9 waits for the camera
    /// move to land and only then shows the art with the gauge reset to `0.0`.
    ///
    /// Negative control: making [`RegionConfirm::advance`] return `ShowCut`
    /// before the timer finishes (art at click time) fails the first two
    /// assertions, and driving the gauge from the flight timer instead of the
    /// load count fails `Loading(0.0)` at the moment the art appears.
    #[test]
    ...
    /// produce. Replacing `plate_alpha_step` with `if hovered { 1.0 } else
    /// { 0.0 }` fails the two `< 1.0` assertions; replacing it with `1.0` fails
    /// the resting assertion.
    #[test]
    ...
    /// different left edges. A fixed screen fraction gives the same left edge
    /// for both, and fails that assertion.
    #[test]
    ...
    ///
    /// Both halves matter: the player-facing sentence comes from the original's
    /// own key and carries no identifiers, while the diagnosis is asserted to
    /// still name the file that would unblock it.
    #[test]
