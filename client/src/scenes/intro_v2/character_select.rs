...
/// A text size scaled like a rect and rounded to a whole pixel: a fractional em
/// rasterises off the pixel grid, which is the blurry half of "font and spacing
/// do not fit".
fn round_text_px(design_px: f32, scale: f32) -> f32 {
    (design_px * scale).round()
}

/// The factor this screen draws its code-placed surfaces at: **1.0**.
///
/// # Why this is not the HUD factor
///
/// `plugins::hud::scale` exists for the *in-game* HUD, whose rects are authored
/// in a 1024x768-era window space and are unreadably small on a modern display
/// (ADR-0009). The original draws this screen 1:1: at 800x600 `hp.ddj` sits at
/// (575,177), `mp.ddj` at y 189 and `info_europe.ddj` at (529,127), all at
/// factor 1.0, while the HUD's 1.5 default would put them at (463,223),
/// (463,241) and (394,148).
///
/// So the HUD factor stays with the HUD, and the pre-game screens draw at the
/// factor the original draws at. The original has no concept of a HUD factor,
/// so there is no authored default to hit.
///
/// **Open:** whether the name plate and the deletion-countdown window sit 1:1
/// in the original too. Only the info box's three anchors are known; the other
/// two follow it here because they are the same class of code-placed surface on
/// the same screen.
fn pregame_scale() -> f32 {
    1.0
}

fn text_px(design_px: f32) -> f32 {
    round_text_px(design_px, pregame_scale())
}

/// The intro font ladder entry for a resinfo `FontIndex`, at
/// [`pregame_scale`].
...
// (`chrome::BAR_H_PCT`). Two consequences:
//
//  * **Right margin 13.** From the formula the Cancel column ends at
//    `W-105+92 = W-13`.
...
//    fixed pixel margin would push the row out of the band on every larger
//    window. [`lower_band_anchor`] reproduces the original's anchor instead.
//
// The three values are deliberately *not* multiplied by any factor: the
...
// the art. If the button art ever honours a factor, these must scale too.
//
...
/// reach this state — a refused List above all — has no other speaker: without
/// this the stage stays empty and silent while only the log knows why. Create
/// and CheckName cannot arrive here, they are answered in the create screen's
/// own state; they are covered by the catch-all rather than named, because
/// being silent about an unexpected action is what this pump exists to prevent.
...
pub fn on_char_selection_action_response(
    ...
    for res in reader.read() {
        ...
        if let Some(err) = res.error_code {
            // The log line stays, and the player gets the other half: the code
            // is resolved against the one lobby error table
            // (`packets::agent::lobby::lobby_error_text` via
            // [`super::lobby_error_line`]) and put on the same status line
            // every other pregame refusal uses.
...
/// one would skip the equipment, and with the rim on the highlight would
/// vanish entirely. The world's own highlight
...
pub(crate) fn info_box(
    ...
    // "EXP" / "Stat" / "Level"; a Korean or German PK2 gets its own words.
    ...
    // The lobby packet carries current HP/MP but no maximum. The maximum is
    // reproducible offline from the same `1.02^(level-1) * stat * 10` curve the
    // server uses, and the lobby's `str`/`int` are the base primaries with no
    // equip or buff modifiers, which is exactly what that curve wants.
    //
    ...
    // `GDR_STA_EXP`/`_SP`/`_LEVEL` (`:166`, `:128`, `:90`). The text carries the
    // same factor as the box it sits in — via `font_px`, which also rounds the
    // result to a whole pixel.
    ...
    //
    // Both offsets carry the same factor as the box itself: scaling the art but
    // not its margins would slide the box off its relationship to the screen
    // edge. With [`pregame_scale`] that factor is 1.0, i.e. the original's own
    // arithmetic, and the box lands on the original's pixels.
...
// the three controls' ids 6/7/8 — in v1.188 EU they are dead code.
//
...
//
// Scaled like this screen's other code-placed surfaces (the
...
/// nameplates get away with the same number because the world camera is much
/// farther away, so the anchor here comes from the figure's own extent rather
/// than from one shared absolute offset.
...
pub fn update_character_name_plate(
    ...
    // figure's own meshes and falls back to the world nameplates' number only
    // while the `.bsr` is still streaming.
...
// window above the lower band. Without it the Restore/Cancel row states a
// choice whose deadline is invisible.
//
...
/// whole design pixel that fits at every scale the tests below check. One
...
///
/// Takes the scale rather than reading [`pregame_scale`] so the tests can walk
/// several scales. The rounding matters and
/// is therefore the *same* rounding the renderer does ([`round_text_px`], which
/// `text_px` is): at a factor of 1.5 a design 9 px becomes 14 device px, not
...
pub fn tick_deletion_countdown(
    ...
    for (entity, mut state) in &mut lines {
        ...
        // `countdown_font_px`, i.e. scaled like the window
...
/// tags. Two tags matter: `<font color="a,r,g,b">…</font>`,
...
pub(crate) fn spawn_deletion_countdown(
    ...
    // What matters is that it is the *same* factor as its neighbour: this
    // window and `info_box` are the same class of control on the same screen,
    // so drawing one of them larger than the other would make the two
    // disagree. Both read [`pregame_scale`], so both are 1:1 like the
    // original. `font_px` (rather than `intro_font_px`) is the same call the
    // box uses: ladder x that factor, rounded to a whole pixel.
    let s = pregame_scale();
...
pub fn spawn_selection_ui(
    ...
    if info.0.is_deleting {
        ...
        // `\n` escapes like its sibling `UIO_STT_CHAR_DEL_CONFIRM`.
...
/// `GDR_BTN_WCANCEL` `178,145,76,32` — one resinfo section, so one builder.
///
...
/// (`UIO_STT_CHAR_DEL_CONFIRM`) over the character's name, with **Delete** —
/// not "Confirm" — on the accept button (`UIO_SELCHAR_CTL_DELETE`).
fn delete_modal(
    character_name: &str,
    assets: &IntroV2Assets,
    fonts: &FontAssets,
...
/// countdown window stays visible behind it.
///
...
pub fn on_character_delete_response(
    ...
    for res in reader.read() {
        ...
        if res.result == 1 {
            ...
            // The refusal speaks the original's own row: the char-select pump
            // answers *any* `0xB007` with
...
/// canvas-scaled from 1600x1200. The numbers and the conversion therefore come
/// from [`crate::scenes::loading_screen`] rather than from a second
/// transcription of them here.
///
...
fn join_loading_overlay(assets: &IntroV2Assets, fonts: &FontAssets) -> impl Scene {
    ...
    // **Zero** of the 247 resinfo files reference `loading_default.ddj`: it is
    // the fallback of the *world* loading screen (the `loading_<region>.ddj`
    // family), a different screen entirely. The four arts below are what this
    // screen authors (`Section = Loading`).
    ...
    bsn! {
        ...
        Children [
            ...
            (
                ...
                // canvas and sits whole in the 4:3 contain box. Stretching it
                // to the *window* squashes the 1024x768 painting on every 16:9
                // screen; *covering* the window crops up to 44% of it away.
...
pub fn on_character_join_response(
    ...
    for res in reader.read() {
        if res.result == 1 {
            ...
            // dispatcher. So a join refusal is rendered by
            // [`super::lobby_error_line`] too, rather than by an invented
            // sentence. A frame with `result` neither 1 nor 2 carries no
...
pub fn despawn_selection_ui(
    query: Query<
        ...
        Or<(
            ...
            With<RestoreConfirmModal>,
            With<DeletionCountdownWindow>,
            // The join *response* (`on_character_join_response`) is not the
            // only way out: leaving the list otherwise — a failed join followed
            // by Cancel, or the scene tearing down mid-flight — would leave a
            // fullscreen loading image on top of the next screen. The overlay
            // belongs to the selection, so it leaves with it.
...
mod tests {
    ...
    /// 1920x1080, where a fixed 16 px bottom margin would miss the band by
    /// 71 px.
    #[test]
    fn the_button_row_follows_the_originals_placement_formula() {
        ...
        // the same formula at a modern resolution — and a constant margin's
        // answer, 71 px lower, for contrast
    ...
    /// against `intro_font_px` so a future factor creeping into the
    ...
    /// The pre-game screens draw at their own factor, and it is 1.0.
    ///
    /// `hud.hud_scale` ships at 1.5 for the in-game HUD. Reading that global
    /// here would draw the lobby's stat box 1.5x while the original draws it
    /// 1:1 (measured at 800x600: `hp.ddj` at (575,177) in the original against
    /// (463,223) at 1.5). The HUD keeps its factor; this screen does not borrow
    /// it.
    ///
    /// The source scan is the load-bearing half: a single `hud_scale()` in one
    /// of the four call sites brings the whole box back, and no value assertion
    /// would see it.
    #[test]
    fn the_pregame_screens_do_not_borrow_the_hud_factor() {
        assert_eq!(pregame_scale(), 1.0);
        // 1.0 means the transcribed numbers are drawn as transcribed.
        assert_eq!(text_px(11.0), 11.0);
        assert_eq!(font_px(2), intro_font_px(2));

        let source = include_str!("character_select.rs");
        // The marker is assembled from two literals on purpose: written out in
        // one piece it would be the *first* thing the scan finds, in its own
        // test module.
        let marker = concat!("#[cfg(", "test)]\nmod tests {");
        let authored = &source[..source.find(marker).expect("the test module")];
        assert!(
            !authored.contains("hud_scale"),
            "a pre-game surface reads the HUD factor again"
        );
    }

    /// The info box's text carries the same factor as the box — and lands on a
    /// whole pixel. An `11.0 * factor` here would give 16.5 px: a fractional
    /// em, and a third under the `FontIndex=2` the box's seven text controls
    /// all declare.
    #[test]
    fn the_info_box_text_scales_with_the_box_and_stays_a_whole_pixel() {
        ...
        // 11 px is not a ladder entry any control of the box declares
        assert_ne!(intro_font_px(2), 11.0);
        assert!(pregame_scale() > 0.0);
    }
    ...
    /// `loading_default.ddj` is referenced by **no** resinfo file — it belongs
    /// to the world loading screen. This screen authors four rect-bearing
    /// controls, so they are percentages of the 1600x1200 canvas, and the frame
    /// must contain the gauge and the label strip.
    ///
    /// Second thing pinned here: the overlay reads the *same* rects as
    /// `loading_screen`, not a second transcription of them. If a future edit
    ...
    fn the_join_overlay_uses_the_authored_loading_rects() {
        ...
        // and the conversion is shared, not a local copy of the percentage math
        assert_eq!(
            design_pct(FRAME_RECT),
            (
                100.0 * 241.0 / 1600.0,
    ...
    /// that the control's own `FontIndex` 0, i.e. the ladder's 12 px, does
    /// **not** fit. Without it the second assertion would pass for any small
    /// number and would prove nothing.
    #[test]
    fn the_countdown_line_fits_its_control_at_every_scale() {
    ...
    /// `0x2760` = 10080 = 7*24*60. The display clamp to the reservation the
    /// copy states keeps a server with a longer policy from overflowing the
    /// line or the gauge.
    #[test]
    fn the_countdown_reads_deletion_time_as_minutes_and_clamps() {
        ...
        // a *seconds*-sized number is not silently re-scaled: 6 days in
    ...
    /// A refusal answered with `error!` **only** leaves the player with an
    /// empty stage and nothing else. `0x0402` (1026) is an appending arm, so
    /// the rendered line must
