...
pub fn on_char_selection_action_response(
    ...
    for res in reader.read() {
        ...
        if res.characters.is_some() {
            ...
            // across the rebuild pointed every consumer
...
/// between the two. A single `caption_color` for all three stood here and
/// painted Level in the EXP gold.
...
mod tests {
    ...
    /// `:152`) — and the original draws both colours in the same box. One
    /// shared caption colour stood here, which painted Level in the EXP gold.
    #[test]
    ...
    ///
    /// `on_char_selection_action_response` despawned the standing figures but
    /// kept [`SelectedCharacterV2`] pointing at one of them. Every consumer —
    /// the info box, the delete countdown, Start and Delete — then queried a
    /// despawned entity: their queries miss, so the box kept the old
    /// character's numbers on screen while the two buttons did nothing at all.
