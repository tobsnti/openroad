...
///
/// Keyed by shard id rather than by a marker on the row entity because
/// [`update_shard_rows`] despawns and respawns every row: scrolling would
/// otherwise drop the highlight, and `Select` reads the highlight, so the
/// player would be one invisible step away from a Connect that does nothing.
/// The id is the only part of a row that is stable across a rebuild.
///
...
/// Index of the first shard row the list shows. The list rect only fits
/// [`VISIBLE_ROWS`] rows, so without an offset a longer shard list is clipped
/// away by `Overflow::clip()` and the three slider buttons have nothing to do.
...
// ---------------------------------------------------------------------------
// Geometry. This whole window is authored in `pstitle_europe.txt` and the rects
// below are that file, verbatim, not a redrawing of it. They are native pixels
// — a window rect in the intro trees is NOT in the 1600x1200 design space the
// two chrome bars use, so it does not scale with the screen. Every number is
// cross-checked against how the original draws this window, and where the check
// is exact that is noted.
...
/// (281,126), i.e. the art carries a 1px transparent margin per side.
...
/// original's selection bar spans x 287..486 — 200px, to the pixel.
...
/// the division so the two rects stay the single source;
/// [`tests::visible_rows_is_the_list_rect_divided_by_the_row`] pins the value.
...
/// `WINDOW_W - (214 + 20)` = 6 is the slider's right margin.
...
/// client). With the pair centred on the screen it puts Select at
...
/// 800x600 client. The frame is not centred vertically, and nothing in
/// `pstitle_europe.txt` says it is: `Rect="0,0,..."` (`:529`) authors the size
/// and leaves the origin to the code.
///
...
/// (125 + 340 + 10 = 475 = the original's Select top).
...
pub fn server_window(
    ...
    // Both buttons get `SND_BUTTON_CLICK` and nothing else; `uiwinclose.wav` has
    // no basis here: `resinfo/effectsound.txt` maps `SND_BUTTON_CLICK` ->
    // `ui\uibutton_a.wav`/`uibutton_b.wav` (`:52`/`:53`) and `SND_WINDOW_CLOSE` ->
    // `ui\uiwinclose.wav` (`:55`), while the original uses `SND_WINDOW_CLOSE` only
    // inside its `IFxxx` HUD windows — **never in the pregame**. The server list
    // is a `CIFStatic` plate, not a closable window, so the click sound alone is
    // the faithful reading. (Paths are written lowercase deliberately: the table
    // says `Error.wav` with a capital E while Data.pk2 ships `error.wav`; our
    // archive index is case-insensitive, `bevy_pk2/src/pk2/archive.rs:66`.)
...
/// `textdata/textuisystem.txt`. The original does not compute these words, it
/// looks them up — and the table ships the thresholds next to the words, so
/// nothing here is a hand-picked fraction. The words are read through
/// [`ClientUiStrings`]: the fallbacks below are the English rows this PK2
/// happens to ship, so a non-English table localizes the cells.
///
/// ```text
...
/// — this PK2 carries 35 and 97 where the Korean defaults are 35 and 75.
...
/// The reading below is the one that uses each word exactly once and matches the
/// key names — `<= 35%` Easy, `<= 97%` Populated, above it Crowded, and FULL
/// only at capacity, where no further login fits.
fn shard_status(shard: &Shard, ui_strings: &ClientUiStrings) -> (Srgba, String) {
    ...
    if !shard.is_operating {
        // `UIO_STT_SERVER_TEST` -> "Check". "Maintenance" appears nowhere in
        // `textuisystem.txt`; positive control on the same read path: "Crowded"
        // is found, as `UIO_STT_SERVER_FULL`.
        return check();
    }
...
fn shard_row(
    ...
    bsn! {
        ...
        Node {
            ...
            // carries the inset, so the row is 200 wide at x=7. The original's
            // selection bar spans x 287..486 with the window art at 280, i.e.
            // exactly 7..207.
            width: px(LIST_W),
            height: px(ROW_H),
...
/// Swaps row art for hover/selection.
...
/// names it.
///
/// **Stated deviation with a rationale.** The original prefills the Server row
/// from `RECENTSERVER` while its *selection* stays empty (see
/// [`update_shard_name_text`]), so its own Connect click sends nothing and says
/// nothing. openroad answers that click with a status line
/// (`net::on_connect_activate`), and that is only half an answer: the player
/// reads a server name in the row, so a hint that says "select a server first"
...
/// [`commit_remembered_shard`], so on a normal start the two agree — and
/// keeping the two states separate is what lets the display survive a
...
pub fn update_shard_name_text(
    ...
    // **No commit -> no name.** A fallback to the remembered `RECENTSERVER`
    // name whenever nothing is committed would show a server name while
    // `SelectedShardV2` is `None`, so Connect would answer "Select a server
    // first" about a server the player can see. Such a fallback only bites when
    // [`commit_remembered_shard`] has refused — remembered shard gone or not
    // operating — i.e. exactly when the name is *not* a selection. Display and
    // state are the same thing by construction rather than by care.
    //
...
/// so this is ours. It runs only while the server-selection screen is up, where
/// this list is the only scrollable surface, so it cannot steal the wheel from
/// anything else.
...
///
/// Its `Activate` handler returns early without a [`SelectedRow`], so an enabled
/// button would be clickable but inert — the original's own `button_disable.ddj`
...
mod tests {
    ...
    /// The frame's origin in the original is (280,125) in an 800x600 client: x is
    ...
    fn the_server_window_sits_where_the_original_does() {
        ...
        // 130 here, and the original draws 125. Neither is derivable from the
        // other, which is why this number is a named constant.
    ...
    /// `SpaceEvenly` over a 288-wide row is the red control: ~35 px apart, and
    ...
    fn select_and_cancel_are_centred_18px_apart() {
        ...
        // 291.33, so `SpaceEvenly` over that row explains a 291 px Select edge
    ...
    /// **The row shows only what is actually selected.** The other way round —
    /// a Server row reading a remembered name while `SelectedShardV2` is `None`
    /// — makes Connect answer "Select a server first" about a server the player
    /// can see.
    ///
    /// That is the original's own trap: it prefills `RECENTSERVER`, selects
    ...
    fn the_server_row_shows_only_a_committed_selection() {
        ...
        // is committed — and the row must stay EMPTY.
    ...
    /// "Select a server first" must not appear while the Server row names a
    /// server. [`commit_remembered_shard`] closes that gap by turning the
    /// remembered name into the selection it looks like — and the two negative
    /// controls are the point of the test: a shard that is **down** and a name
    /// that is **not in the list** stay uncommitted, so the hint is only ever
    ...
    /// row rebuild that every scroll step performs. A marker on the row entity
    /// would be gone after scrolling.
    #[test]
