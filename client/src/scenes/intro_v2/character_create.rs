...
// reason.
//
// One unit for the whole pre-game: if the pre-game is ever scaled, it is scaled
// in one place for all its screens, not here.
...
// mid-'n'. `45` was never a transcription error, and it should not be "fixed".
//
...
// `plugins::hud::scale::FONT_INDEX_PX`).
...
fn create_panel(
    ...
    // inside the 45px box instead spreads the column by up to 8.7 px, i.e. a
    // ragged left edge.
    ...
    bsn! {
        ...
        Children [
            ...
            // template as the other three. The nibble assignment is known,
            // not guessed: from the `0x22` default, Height +1 sends `0x23` and
            // Volume +1 `0x32`, so low = Height, high = Volume.
            (
...
/// 600 — **82.0 %** of the client height. Error band: 79.4..84.2 %.
...
/// customise window (ends at x=335) and the info box (starts at x=575).
...
///
/// Measuring the body instead of dollying by a fixed ratio is what makes this
/// resolution- and body-independent: apparent size is only ~1/distance for a
/// *point* — a body half as far away grows by more than its distance ratio.
...
/// own yaw rather than from a constant: a stage whose camera looks the other
/// way must still show the figure's face, not its back. The negation is the
...
pub fn update_preview(
    ...
    // The preview is drawn at the model's own size. The wire scale byte is not
    // a linear 0..255 scale: it is two independent nibbles of five steps each
    // (see [`DEFAULT_SCALE`]/`pack_scale`). Which nibble is which is known
    // (low = Height, high = Volume), but the per-step *visual* delta is not in
    // any data we hold, so the honest preview stays the unscaled body; the
    // mirror stays (the podium lineup is mirrored on x).
    let scale = 1.0_f32;
...
/// Applying the mechanical swap alone therefore resolves to nothing for nine of
/// the twelve European weapons and draws an **empty description box** for a
...
pub fn on_check_name_activate(
    ...
    // in that sink is the answer ("Valid ID" / the mapped error).
    text.0 = String::new();
}
...
fn create_confirm_modal(
    ...
    // the scrim decision (an openroad convention, not the original's) lives in
    // one place.
    bsn! {
        modal_scrim()
        CreateConfirmModal
...
mod tests {
    ...
    fn the_framing_only_moves_along_the_authored_view_axis() {
        ...
        // and it moved *closer*, as the larger on-screen height requires
    ...
    /// The row is the same 1:1 art at every window size, like the login form
    /// and the region plates.
    #[test]
    ...
    /// so the customize panel is the original's 264x316 at 1600x900 too.
    #[test]
    ...
    /// `UIO_NEWCHAR_EXPLANATION_EU_TWOHANDSTAFF`, which the mechanical key
    ...
    fn the_customize_panel_children_are_the_authored_resinfo_rects() {
        ...
        // The caption column is deliberately *not* the authored x/w
    ...
    fn rotating_right_then_left_returns_to_the_podium_pose() {
        // A camera looking down -Z (yaw 0): the body faces it.
        ...
        // around with it, instead of showing its back.
    ...
    fn the_scale_byte_is_two_nibbles_and_the_default_is_the_middle_step() {
        ...
        // 1.0 + 0x22/255 is not a scale factor
        assert!(
            DEFAULT_SCALE < 0x55,
    ...
    /// `0x32`. A swap of these two lines would mint mis-shaped characters, so
    /// it gets a test rather than only a comment.
    #[test]
    ...
    /// window is exactly 2..=12. Its upper end is unreachable: 15 typed
    /// characters become 12 in the field and no message appears, so the cap
    /// belongs on the input and only the lower end is a message. This test pins
    /// both halves.
    #[test]
