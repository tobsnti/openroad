...
/// Arrival pose: the direction is the original's, the distance is ours
/// (stated deviation, ADR-0009).
///
...
/// We keep the azimuth `yaw 2.53` and the magnitude `|pitch| 0.62`, flip the
/// elevation's sign so the camera looks down at the figures, and take the
/// distance from whatever frames the idols at [`BOARD_FRAME_FILL`]. Framing
/// the whole desk instead puts the camera ~39 units out; framing the idols
/// lands ~17 units out, within 10 % of the original's own stand-off.
...
/// because that constant is private to the camera plugin.
...
/// fill. Our number (ADR-0009): `0.85` keeps every corner of the motif inside
/// the viewport at both `21:9` (3440x1440) and `16:9`.
...
/// the last one turns onto the idols.
///
/// Waypoint 0 is deliberately not used. Its own rotation looks away from both
/// the lineup (dot `-0.73`) and the idols (`-0.23`); it is the entry the
/// t-ladder puts at `t = 5.0`, i.e. at the far end of the path, and our stage
/// asset authors its own lineup pose that the player is already looking
/// through. So the flight starts from the live camera pose and walks the
/// original's three waypoints from there — no invented pose, and no cut.
...
///
/// Why this is a stated deviation and not an invented number (ADR-0009): the
/// original's region for the pregame stage is unknown, our
/// `constantinople.selection` is openroad-authored, and the board coordinates
/// are region-local, so on our region the tableau lands 6.3 units below our
/// pier. Lifting the group rigidly keeps every relation — prop-to-prop and
/// camera-to-prop — exactly as the original has it, and changes only which
/// piece of ground the stage stands on.
...
/// observable — the tween itself keeps its end pose private.
...
/// [`board_table_top`] for an arbitrary box facing. Split out so a caller can
/// run the same mapping against another facing without a second copy of it.
...
/// flipped — see [`BOARD_CAM_PITCH`]). Distance comes from the motif: put the
/// props' bounding sphere at [`BOARD_FRAME_FILL`] of the frame's half-height,
/// i.e. `radius / (fill * tan(fov/2))`, with the same vertical FOV the
/// cinematic camera carries (`plugins::camera::FOV`, 45°). Aiming at the
/// sphere's centre is what centres the tableau.
pub fn board_camera_pose(cam_base: Vec3, origin: Vec3) -> (Vec3, Quat) {
    ...
    // The scene asset's own keyframes use `PI - yaw` instead, because
    // `CameraKeyframe` mirrors translation and rotation together; a hand-built
    // pose mirrors a yaw to `-yaw`.
...
///
/// The original's click is a 1.0 s pure translation of 3.65 (europe) / 3.84
/// (china) units with the orientation unchanged, and its purpose is to put the
/// chosen idol on the view axis — each region's target minimises its own
/// idol's off-axis angle (6.23° → 2.82° europe, 8.79° → 5.21° china) and
/// worsens the other's. It ends up further from the figure than it started,
/// because the original already stands 18.2 units inside its tableau.
///
/// Our arrival frames the idol group from above ([`BOARD_CAM_PITCH`]), so that
/// delta transplanted into this shot reproduces nothing. The click applies the
/// original's decision through the rule this file already states — same
/// direction, same [`BOARD_FRAME_FILL`], motif = the chosen idol's own box
/// instead of the idol group. The distance falls out of the data (the idols
/// are ~1.1 x 2.2 x 1.1, so it lands ~3.8 units out): a close-up of the figure
/// the player picked, with no new constant (stated deviation, ADR-0009).
...
/// rotation. Returns the pose it is flying to, so the caller can see the
/// destination.
...
/// `(from 0, to 255, duration 1.0 s, delay 0.0, flag 1)` — the same second
/// the camera move takes — so its own end pose is essentially invisible and
/// the loading art arrives on black. Setting this to `false` shows the move
/// instead; that is a taste call, and fidelity wins.
///
/// The flight is not dead code behind this switch. It sets the pose the camera
/// stands on when the black lifts: the close-up of the chosen figure the
/// creation screen inherits (`CPSCharacterCreate*` authors no camera of its
/// own). Disable the move and the state after the fade is wrong, invisibly.
...
struct BoardProp {
    ...
    ///
    /// The hover test in state 8 walks `i = 0,1` only over the prop array, so
    /// exactly two of the three loaded props are pickable, in the array's own
    /// order — prop 0 `interface_idol_europe.bsr` sets `g_region = 0`, prop 1
    /// `interface_idol_china.bsr` sets `g_region = 1`, and the same index
    /// selects the plate and the loading art in state 9. The box and the lizard
    /// are scenery: the lizard is the Arabia idol of a race with no player body,
    /// and because it is not pickable its animated root displacement
    /// (`anim_offset`) never enters a hit path.
    race: Option<Race>,
    offset: Vec3,
    ...
    /// header field 5 `bounding_box_offset`, six floats). It is here because the
    /// arrival pose is derived from the props instead of being a free pose: the
    /// camera has to know how big its motif is, and the motif's size is data.
    ///
    /// Taken from the shipped meshes rather than from the `.bsr`'s own record,
    /// which does not always agree with any mesh (see the `interface_lizard`
    /// row). `box.bsr` is not a crate but a five-mesh desk (`interface_box`
    /// plus `interface_compass`, `interface_idol_brushcase`, `interface_letter`,
    /// `interface_map`), so its union's top is the letter's top, not the table
    /// surface — the surface is [`BOARD_TABLE_TOP_LOCAL_Y`].
    bbox_min: Vec3,
    bbox_max: Vec3,
...
const BOARD_PROPS: [BoardProp; 4] = [
    BoardProp {
        ...
        // The box is not left at the default facing: right after its own
        // `SetPosition` the same object is handed the same one-float setter the
        // three idols get, with 3.0 (the idols get {3.31, 3.03, 3.0}).
        ...
        // land behind it. With this value all three stand on the table.
        yaw: 3.0,
        motif: false,
    },
    BoardProp {
    ...
    // for. It is part of the scenery the camera frames, so it is placed like
    // the other two; what stays absent is its 2D plate, which the data does not
    // offer either.
    BoardProp {
        ...
        // The mesh union and the animated root displacement below are kept
        // apart on purpose, which is what makes both numbers checkable: the
        // mesh's size comes from the mesh, the displacement from the clip.
...
/// floating origin and then never move, and a second derivation of the same box
/// (one for the arrival framing in [`board_props_bounds`], one for the pick)
/// could drift apart.
...
/// screen. Our placement, not the original's: the row is continued by the step
/// between the two authored idols (`interface_idol_china.offset -
...
///
/// The race board is a place, so it has to be furnished: the models are the
/// user's own (`Data.pk2 → res/interface/…`), spawned through the same
...
///
/// Why a path and not a straight line: the original's path swings past the
/// character line first (both middles look straight at it, see
/// [`BOARD_WAYPOINTS`]) and only turns onto the idols at the end, which is the
/// shot the original composes. A straight line approaches the boards head-on
/// from below with nothing in frame on the way.
fn board_path_poses(
    cam_base: Vec3,
    origin: Vec3,
    reverse: bool,
...
/// pose ladder, same `5/3 s`, no new number. The outbound leg is a flight, so
/// an instant snap back would be a visible asymmetry.
#[derive(Resource)]
...
mod tests {
    ...
    ///
    /// It also settles the rotation convention: `XYZ(pitch, PI - yaw)` scores
    /// -0.30 against this direction (facing away), `YXZ(-yaw, pitch)` scores 0.988.
    #[test]
    fn the_board_camera_looks_at_the_idols() {
        let (pos, rot) = board_camera_pose(Vec3::ZERO, Vec3::ZERO);
        // Through the prop mapping, not by hand: the lift is rigid, so camera
        // and idol have to go through it on both sides or the comparison
        // carries 6.3 units of built-in error.
        // The mesh, not the placement point: `interface_idol_china.bms` sits
        // ~5 units off its own origin (bbox x 3.2..4.8, z -3.9..-2.3), which
        // from the close arrival is a 15° error.
    ...
    /// as a comparison, not as an absolute threshold.
    ///
    ...
    ///
    /// Without the lift this assertion fails (`y = -29.6` against `-35.9`),
    /// which is what keeps the constant honest.
    #[test]
    fn the_pose_follows_the_scene_mapping() {
        ...
        // the pose keeps its separation from the props (which go through the
        // same mapping). A formula copy would only assert that the line was
        // copied correctly.
    ...
    /// BOTH aspect ratios, with a negative control on the desk-framing pose.
    ///
    /// This is a picture criterion, not a dot product. A dot product only says
    /// the camera points somewhere near the motif; it can score `0.988` while
    /// the tableau sits in the bottom-left corner of the frame. So the assertion
    ...
    ///
    /// Negative control, in the same test: the same rule applied to the whole
    /// desk must leave the idols small.
    #[test]
    fn the_arrival_frames_the_props_at_both_aspects() {
        ...
        // Framing the whole desk stands ~39 units off the idols, which is too
        // far away; the original stands 18.2 off its tableau. The idols are the
        // motif, and the arrival must be in the original's range.
        ...
        // Negative control: the same rule applied to the union of all four
        // props (desk included) stands more than twice as far out and shrinks
        // the idols to under a sixth of the frame height.
        let mut min = Vec3::splat(f32::MAX);
        let mut max = Vec3::splat(f32::MIN);
    ...
    /// The negative control at the end is what makes the check meaningful: with
    /// the box unrotated (`yaw = 0.0`) the very same check fails.
    ///
    ...
    fn every_idol_stands_on_the_table() {
        ...
        // NEGATIVE CONTROL — the box unrotated. Two of the three idols then
        // land behind the slab (the europe idol 2.6..3.8 units past its back
        // edge, in mid-air over the stone step), so at least one footprint
        // corner must fall off the table. If this stops failing, the check
    ...
    /// carries: each region's target minimises the off-axis angle of its own
    /// idol, in the ORIGINAL's frame and with the original's own forward
    /// vector. If either triple were mis-transcribed — or read in memory order
    /// instead of coordinate order — this table collapses.
    #[test]
    ...
    /// OUR confirm pose: it puts the chosen figure on the view axis.
    ///
    /// The two frames are kept apart on purpose. The original's behaviour is
    /// pinned one test up, `the_confirm_targets_centre_their_own_idol`: a 1.0 s
    /// pure translation that ends further from the figure while centring it.
    /// Ours additionally closes in, because our arrival stands 39.1 instead of
    /// 18.2 units back and therefore has to — so the distance assertion below
    /// is a statement about our stated deviation, and the off-axis assertion is
    /// the decision both share.
    ///
    /// The assertion pair is its own negative control: with no flight the
    /// confirm pose is the arrival pose, so every `<` below becomes a `==` and
    /// the test fails. It also fails if the pose stops depending on the race (a
    /// single confirm shot for both), or if it aims past the figure.
    #[test]
