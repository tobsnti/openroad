...
//!
//! Deliberate deviation with a stated rationale (ADR-0009); the original's
//! silence here is a known absence rather than an unknown. The original v1.188
//! does not pose its lobby figures:
//!
...
/// Preview hook, `PREVIEW_DELETING=sit|down|up`: pose every lobby figure as if
/// its deletion were reserved.
///
...
///
/// `stop_all` first, and not for tidiness: `spawn_resource` starts the looping
/// stand (`commands/mod.rs`), and Bevy blends every active animation by weight,
/// so a second clip played on top at weight 1.0 does not replace the stand — it
/// averages with it, and the seated pose reads as a stand with slightly bent
/// knees. The same trap is written up in [`crate::plugins::animation_sit`]'s
/// module doc for the world's wrappers.
