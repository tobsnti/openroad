...
pub struct IntroV2Assets {
    ...
    // actually ships: `ls Media/interface/outer` has `list_button.ddj`,
    ...
    // unrelated names. That convention is the rule, not a coincidence — of the
    // 19 files in this PK2 whose name ends in `_europe`, 15 have exactly that
    // bare counterpart (`button`, `login_window`, `customize_window`, `info`,
    // `pstitle`, `pscharacterselect`, the four `blackbar`s, …).
    ...
    // styling per race, not a misnamed file. Loading only the European pair
    // therefore put a teal band on the Chinese screen; the bug is a convention
    // left unmodelled, not a file mixed up. See `chrome::create_bar_art`.
    ...
    // scene now resolves a `UIO_*`/`UIIT_*` key through `ClientUiStrings` with an
    ...
    // `delete_time_gauge_up.ddj`), which is what unblocks the window that until
    // now drew no plate and substituted two flat rects for the gauge.
    ...
    /// Loaded but no longer drawn by the lobby: the join overlay used to draw
    /// this (`character_select::join_loading_overlay`), and the data says it
    /// should not: no `resinfo` file references `loading_default.ddj`, while all
    /// four controls of the lobby's own `Section = Loading` name the arts above.
    ...
    /// fallback; the lobby no longer reads it.
    ...
    // drift apart unnoticed; the remaining direct `AudioPlayer::new(assets.
    // sound_*)` call sites in this scene are the next thing to route through
    // the registry.
    //
    ...
    // **Deliberately loaded but not played anywhere in the pregame**, and that is
    // on purpose, not an oversight. In the original the window-close sound belongs
    // to the `IFxxx` HUD window classes; the pregame never plays it, which is why
    // `server_select.rs:162-171` no longer stacks `uiwinclose.wav` on its button
    // click. The pregame's plates are `CIFStatic` art, not closable
    // windows, so wiring this up would invent a sound the original does not
    // play. The handle stays because the collection mirrors the four UI sounds
    // the intro's asset set declares; if a pregame window ever *does* close, it
    // needs a call site plus a reason, not just this field.
