...
//! `Section = CreateChina`) and in five arts besides the bands. That also
//! explains the 4th `redbar` reference below, which used to look redundant.
//!
//! Searching for `blackbar` alone finds 13 references and none in the create
//! trees, which reads as "the create screen has no chrome". It has one — in red.
//! Across the resinfo files there are 16 `interface\outer\*bar*` references: 13
//! `blackbar` + 4 `redbar`. All eight arts are 1600x172 ARGB1555 and distinct.
//!
...
/// (`y = barDown.top + 27`, `y = barUp.bottom + 41`, `character-select-spec.md`
/// §1.5), so `character_select` anchors against this same percentage instead of
/// re-deriving it from a second copy of 172/1200.
...
/// Orange on the create screen is the same defect [`update_chrome_art`] was
/// written for on the bars: the notice line is spawned once for the whole
/// scene, so without a per-tree swap it keeps the title tree's colour where
/// the data says white — and it sits on the RED bar there, which is where the
/// original's own contrast argument for white shows up.
...
/// `FontIndex=0` does now: the ladder behind [`intro_font_px`] resolves index 0
/// to **12 px** (`GDR_TEXT_MESSAGE`, `pstitle_europe.txt:634` and
/// `pscharacterselect_europe.txt:497`, both `FontIndex=0`). The previous 16 was
/// an openroad guess made while "FontIndex -> size" was still UNKNOWN — it is a
/// third too large, which is why this line looked like a different font from the
/// rest of the intro. The two placement numbers below are unchanged.
...
/// edge.
///
/// The earlier `BAR_H_PCT + 2.0` came from the right diagnosis and the wrong
/// cure: at 10% the text *was* painted over, but only because the bar art is its
/// own UI root and stacked later. [`NOTICE_Z`] is that fix. Lifting the line out
/// of the band on top of it moved it 40 px off the original for no stated
/// reason.
...
/// what keeps the line readable now that it sits *inside* the bar band again.
...
/// and modelling only one half is how the Chinese stage ended up in teal.
///
...
///
/// # The idea — why the font handle is not optional here
///
/// This entity used to be the
/// only `TextFont` under `client/src/scenes/` without a `font:` handle, so Bevy
/// fell back to its embedded default face `FiraMono-subset.ttf`
/// (`bevy_text-0.19.1/src/`). That subset has **no glyph for U+2026**: a cmap
/// lookup on it answers gid 0 (`.notdef`, i.e. the tofu box) for U+2026 while
/// `A` resolves to gid 36 — ASCII is in, the ellipsis is not. And the string the
/// connect path puts here is `UIO_MSG_ERROR_CITATION`
/// (`textdata/textuisystem.txt:190`), whose cell is literally
/// `U+2026 "Requesting user confirmation" U+2026` — an ellipsis on *both* ends.
/// Hence the two tofu boxes around that line.
///
...
/// 2157), so handing the line the same `fonts.nine` the rest of the intro uses
/// fixes the tofu *and* the second half of the same defect: the status line was
/// rendering in a monospace face while every other word on the screen was in the
/// UI face.
///
/// # The idea — why the node has a one-line height
///
/// The line is anchored by its *bottom* edge ([`NOTICE_BOTTOM_PCT`], which is a
/// one-line value), so a message with three lines used to grow **upwards**: its
/// first line landed two line heights above where every one-line message sits,
/// which is why the deletion notice sat too high, on a different y than the rest
/// of the screen.
///
...
///
/// Fixing that without moving the anchor: the node keeps its
/// bottom edge and gets the height of exactly one line, so its *top* edge — and
/// with it the first baseline — stays where the one-line case puts it,
/// and further lines overflow downwards over the band, as they do in the
/// original. `1.2 x font size` is Bevy's own default line height
...
mod test {
    ...
    /// [`NOTICE_Z`]'s job, not the layout's — the previous
    /// `NOTICE_BOTTOM_PCT > BAR_H_PCT` assertion encoded the opposite belief.
    #[test]
    ...
    /// Chinese tree (the European create tree pairs the same role with a
    /// teal art; see [`create_bar_art`]). A
    /// search for `blackbar` alone misses that entirely, and the two black pairs
    ...
    /// Half one of the defect where "Out of service area." was still standing on
    /// the next screen: a sentence written for one screen must not survive the
    /// change. The notice line is one entity for the whole intro scene,
    /// so nothing else would ever remove it — this is red without the clear in
    /// [`update_info_text`].
    #[test]
    ...
    /// Half two, and the one that keeps the fix from repeating the captcha
    /// mistake (#878, a screen left mute): a message written *for the state
    /// being entered* — `OnEnter` runs in `StateTransition`, before `Update` —
    /// must survive the clear in the same frame. Without the ordering argument
    /// in [`update_info_text`] this is exactly what a clear-on-enter would eat.
    #[test]
    ...
    /// check below is a *cross-file* one — which is the only place the defect
    /// could live, because each file on its own was already self-consistent:
    /// `assets.rs` happily declared one pair, and `chrome.rs` happily used it
    /// for both races.
    ...
    /// files. Before this fix only the European pair was declared, so the
    /// Chinese stage was framed in the European art.
    #[test]
    ...
    /// no `_europe` art for must
    /// name the fields that carry the files **without** the `_europe` suffix,
    /// and vice versa. `pscharactercreatechina.txt:253/234` names
