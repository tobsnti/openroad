...
///
/// Why the env door exists: reaching a post-login screen needs a login, and the
/// only switch used to be `config.yaml`, i.e. the user's own file. Leaving it on
/// there is easy to forget, and the client then drives past the login form into
/// the world on every ordinary start. An env var is the same class of dev-only
/// hook as [`jump_target`]'s `OPENROAD_INTRO_JUMP`: inert unless set, and it
...
pub fn jump_to_requested_screen(
    ...
    // before jumping.
    //
    // Why it exists: the jump used to fire on the very response that spawns the
    // figures, and the state switch happens before
    // `character_select::on_char_selection_action_response` — which only runs
    // *in* `CharacterList` — ever gets to spawn them, so a jump run left the
    // stage empty. A delay is the smallest thing that keeps the figures on
    // stage while the board flight runs.
...
/// fires; `0.0` (the default) keeps the previous behaviour.
