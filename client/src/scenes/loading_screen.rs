...
impl Plugin for LoadingScenePlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(UiMaterialPlugin::<LoadingBackdropMaterial>::default())
            ...
            // in the headless harnesses (rule 7). `attach_backdrop_material`
            // rides along for the same reason: the backdrop is spawned on all
            // four surfaces and only this plugin owns its material store.
...
///
/// DEVIATION, deliberate: the original gives no answer for a non-4:3 window, and
/// this screen used to stretch both art and chrome to the window, which squashed
/// the painting and the "now loading" strip on every 16:9 screen. So we keep the
/// ratio instead of inventing one: painting *and* chrome sit in the largest
/// centred 4:3 box that fits (*contain*), which reproduces the authored
/// proportions at any window size.
///
/// Letting the painting *cover* the window instead only trades one defect for
/// another — it looks zoomed in: at 21:9 cover eats ~44% of the picture, and
/// being uncropped in one axis does not make that the whole painting. What
/// replaced it is the third box: the leftover
/// area is not black bars but the *same* picture blown up to cover, blurred
/// and dimmed ([`BACKDROP_BLUR`], [`BACKDROP_BRIGHTNESS`]), so the painting
/// ends in its own colours. Nothing of the art is lost and nothing is
/// stretched — the deviation buys back what cover threw away.
...
/// scene resource it cannot go missing (rule 7).
...
/// headless harnesses (rule 7: no scene resource, nothing to fail parameter
...
/// `ChildSpawnerCommands` helper, and it used to carry its own copy of the
/// three rects *and* its own percentage math. Sharing the conversion keeps the
/// two renderers from drifting even though they cannot share the spawn code.
...
mod test {
    ...
    /// The loading screen used to stretch with the window.
    ///
    ...
    ///
    /// The RED control is the second half of the test: the same
    /// arithmetic against the full window — what the screen did until today —
    /// misses 7.2 by 33% at 16:9 and by 79% at 21:9. Without it a green here
    /// would only be saying that 4:3 is 4:3.
    #[test]
    fn the_caption_keeps_the_arts_aspect_in_a_design_box_and_not_in_the_window() {
        ...
        // RED control: the old full-window placement
    ...
    ///
    /// The number that made cover the wrong answer, and the reason the third
